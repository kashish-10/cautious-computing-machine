        ...
        { liveNewsData.length >= 10 ? (
          <Container className='new-news-container'>
              ...
              <Col key={index}>
                <div>
                  <div className='div-ipsdf'>
                    {liveNewsData[index]?.img_url !== 'None' && (
                      <Row>
                        <img src={liveNewsData[index].img_url} width={500} height={200} alt={`Image ${index}`} />
                      </Row>
                    )}
                    <Row>
                      <h5>{liveNewsData[index]?.title}</h5>
                    </Row>
                  </div>
                  <div className='div-kjpql'>
                    <div>
                      {`${new Date(liveNewsData[index]?.publication_date).getDay()}/${new Date(liveNewsData[index]?.publication_date).getMonth()}/${new Date(liveNewsData[index]?.publication_date).getFullYear()} ${new Date(liveNewsData[index]?.publication_date).getHours()}:${new Date(liveNewsData[index]?.publication_date).getMinutes()}`}
                    </div>
                    <div className='div-kpqsa'>
                      {liveNewsData[index]?.prediction === true ? (
            ...
            {/* <Col>
                <div>
                  <div className='div-ipsdf'>
                    { liveNewsData[0].img_url === 'None' ? null : (
                      <Row>
                        <img src={liveNewsData[0].img_url} width={500} height={200}></img>
                        </Row>
                    ) }
                    <Row>
                        <h5>{liveNewsData[0].title}</h5>
                    </Row>
                  </div>
                  <div className='div-kjpql'>
                    <div>
                      { new Date(liveNewsData[0].publication_date).getDay()}/{new Date(liveNewsData[0].publication_date)
                      .getMonth()}/{new Date(liveNewsData[0].publication_date).getFullYear()} 
                      {new Date(liveNewsData[0].publication_date).getHours()}:{new Date(liveNewsData[0].publication_date)
                      .getMinutes() }
                    </div>
                    <div className='div-kpqsa'>
            ...
            <Row className="news-row-2">
              {liveNewsData.slice(4, 8).map((news, index) => (
                <Col sm key={index}>
            ...
            <Row className="news-row-2">
              {liveNewsData.slice(8, 12).map((news, index) => (
                <Col sm key={index}>
                  <div className="div-olapq">
                    {news.img_url !== 'None' && (
                      <Row>
            ...
            <Row className="news-row-3">
              {liveNewsData.slice(12, 16).map((news, index) => (
                <Col sm key={index}>
                  <div className="div-olapq">
                    {news.img_url !== 'None' && (
                      <Row>
